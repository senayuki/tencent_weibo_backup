...
class tencent_weibo:
	def __init__(self):
		util_favor.add_log('favor备份启动')
		self.uname = input("请输入用户ID：@")
		print ("你输入的用户ID是: " + self.uname)
		util_favor.add_log('用户名是'+self.uname)
		self.page_index = 1
	...
	def login(self):
		...
		if self.isElementExist(".map_404"):
			print('ID不存在！')
			util_favor.add_log('网页发生404错误，退出')
			exit()
		else:
			try:
				try:
				...
				try:
					...
					self.c.execute('''CREATE TABLE FAVOR
						   (CID INT NOT NULL,
						   TID INT NOT NULL,
						   TYPE INT NOT NULL,
					...
					self.conn.commit()
					self.c.execute('''CREATE TABLE FIMAGE
						   (CID INT NOT NULL,
						   RANK INT NOT NULL,
						   URL TEXT NOT NULL);''')
					self.conn.commit()
					...
					self.conn.commit()
					print('创建表成功')
					util_favor.add_log('创建库成功')
				except:
					print('找到旧表')
					util_favor.add_log('找到旧数据库')
				try:
					...
					for row in latest_url:
						if row[1]!="":
							...
							util_favor.add_log('找到上次的URL：' + row[1])
							start_url = row[1]
							tmp = input('请选择是否使用：0.使用 1.不使用\n')
							if(tmp!="0"):
								self.browser.get('http://t.qq.com/favor?')
							else:
	...
	def click_next_page(self):
		...
		try:
			pi = int(util_favor.get_pi(self.browser.current_url))
			print(pi)
			if(pi<6):
				next_page = pageNav.find_element_by_xpath('/html/body/div[1]/div[2]/div/div[1]/div[3]/a[10]')
			if(pi>5 and pi<97):
				next_page = pageNav.find_element_by_xpath('/html/body/div[1]/div[2]/div/div[1]/div[3]/a[11]')
			if(pi==97):
				next_page = pageNav.find_element_by_xpath('/html/body/div[1]/div[2]/div/div[1]/div[3]/a[10]')
			if(pi==98):
				next_page = pageNav.find_element_by_xpath('/html/body/div[1]/div[2]/div/div[1]/div[3]/a[9]')
			if(pi==99):
				next_page = pageNav.find_element_by_xpath('/html/body/div[1]/div[2]/div/div[1]/div[3]/a[8]')
			if(pi==100):
	...
	def get_items(self):
		for story in self.stories:
			...
			try:
				...
				print('出现灵异事件？')
				print('----------------------------------------------------------------------------------')
				util_favor.add_log('出现灵异事件'+self.cid)
				continue
			self.content = util_favor.analyse_content_html(self,story)
			if (self.content == '')and(self.author == '(@)'):
				util_favor.add_log('找到一条原文已删除的收藏：'+self.cid)
				continue
			else:
				self.time, self.qtime = util_favor.get_time(story)
				...
				print('时间：', self.time)
				#心情模块
				self.mood = util_favor.get_mood(self, story)
				#视频模块
				util_favor.analyze_video(self,story)
				if self.type!=1:#转评作者内容时间
					try:
						self.qauthor , self.qcontent = util_favor.get_quotation_html(self,story)
					except:
						self.qauthor = ''
						self.qcontent = ''
					if self.qtime !="":
					...
					self.qauthor = ""
					self.qcontent = ""
				util_favor.get_image(self,story)
				self.location , self.longitude , self.latitude = util_favor.get_loc(story)#定位
				util_favor.sql_insert(self)
				print('----------------------------------------------------------------------------------')
	...
	def start(self):
		while self.have_next_page:
			...
			if self.last_url != self.browser.current_url:
				...
				self.conn.commit()
				util_favor.add_log('开始分析URL：'+self.browser.current_url+' 这是分析的第'+str(self.page_index)+'页')
				self.page_index += 1
				print('----------------------------------------------------------------------------------')
				self.get_stories()
				self.get_items()
				...
				util_favor.add_log('发生一次页面重复'+self.browser.current_url)
				try:
					print('试图刷新解决')
			...
			if (self.page_index-1) % 100 == 0:
				...
				util_favor.add_log('备份完成，本次共计备份'+str(self.page_index-1)+'页。')
