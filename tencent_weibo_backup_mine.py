		...
		if self.isElementExist(".map_404"):
			...
			try:
				...
				try:
					...
					self.c.execute('''CREATE TABLE MINE
						   (CID INT NOT NULL,
						   TID INT NOT NULL,
						   TYPE INT NOT NULL,
				...
				try:
					...
					for row in latest_url:
						if row[1]!="":
							...
							util.add_log('找到上次的URL：' + row[1])
							tmp = input('请选择是否使用：0.使用 1.不使用\n')
							if(tmp!="0"):
								self.mode = input('请选择模式：0.广播 1.原创 2.转播 3.评论\n')
								util.add_log('放弃使用上次的进度，选择了模式'+self.mode)
								self.c.execute("UPDATE INFO set VALUE = '"+self.mode+"' where ITEM=='LATEST_MODE'")
								self.browser.get(current_url + '/mine?filter='+self.mode)
								self.conn.commit()
							else:
								start_url = row[1]
								latest_mode = self.c.execute("SELECT * FROM INFO WHERE ITEM=='LATEST_MODE'")
								for row in latest_mode:
									self.mode = row[1]
									print("上次的模式：", row[1])
							...
							self.mode = input('请选择模式：0.广播 1.原创 2.转播 3.评论\n')
							util.add_log('数据库中无旧数据，选择了模式'+self.mode)
							self.c.execute("UPDATE INFO set VALUE = '"+self.mode+"' where ITEM=='LATEST_MODE'")
							self.browser.get(current_url + '/mine?filter='+self.mode)
							self.conn.commit()
				except:
	...
	def start(self):
		while self.have_next_page:
			...
			if self.last_url != self.browser.current_url:
				...
				self.conn.commit()
				util.add_log('开始分析URL'+self.browser.current_url+' 这是分析的第'+str(self.page_index)+'页')
				self.page_index += 1
				print('----------------------------------------------------------------------------------')
				self.get_stories()
				self.get_items()
				...
				util.add_log('发生一次页面重复'+self.browser.current_url)
				try:
					print('试图刷新解决')
			...
			if (self.page_index-1) % 100 == 0:
				...
				util.add_log('备份完成，本次共计备份'+str(self.page_index-1)+'页。')
