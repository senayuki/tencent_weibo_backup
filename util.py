...
def get_quotation_html(self, story):
	try:
		...
		con = quotation_block.text
		qauthor = str("")
	except:
		qauthor = self.browser.find_element_by_xpath('//*[@id="'+self.cid+'"]/div[3]/div[3]/div/div[1]/strong/a[1]').get_attribute('title')
		try:
			#html分析
			qauthor_html = self.browser.find_element_by_xpath('//*[@id="'+self.cid+'"]/div[3]/div[3]/div/div[1]/div').get_attribute('innerHTML')#先得到HTML版本的信息
			con = ''.join(c for c in qauthor_html if ord(c) >= 32)#删除控制字符
			...
			con = self.browser.find_element_by_xpath('//*[@id="'+self.cid+'"]/div[3]/div[3]/div/div[1]/div').text
...
def sql_img(self,img_url):
	...
	self.excursor = self.c.execute("SELECT * FROM IMAGE WHERE CID == '"+self.cid+"' AND URL == '"+img_url+"'")
...
def sql_insert(self):
	flag=False#判断数据库中是否已存在
	self.excursor = self.c.execute("SELECT * FROM MINE WHERE CID == " + self.cid)
	for row in self.excursor:
		flag=True
	if flag==False:
		try:
			self.c.execute("INSERT INTO MINE (CID,TID,TYPE,AUTHOR,CONTENT,TIME,QAUTHOR,QCONTENT,QTIME,MOOD,LOCATION,LONGITUDE,LATITUDE) VALUES ("+self.cid+","+self.tid+","+str(self.type)+",'"+self.author+"','"+self.content+"','"+self.time+"','"+self.qauthor+"','"+self.qcontent+"','"+self.qtime+"','"+self.mood+"','"+self.location+"','"+self.longitude+"','"+self.latitude+"')");#用''引
		except:
			try:
				...
				sql_error = 'INSERT INTO MINE (CID,TID,TYPE,AUTHOR,CONTENT,TIME,QAUTHOR,QCONTENT,QTIME,MOOD,LOCATION,LONGITUDE,LATITUDE) VALUES ('+self.cid+','+self.tid+','+str(self.type)+',"'+self.author+'","'+self.content+'","'+self.time+'","'+self.qauthor+'","'+self.qcontent+'","'+self.qtime+'","'+self.mood+'","'+self.location+'","'+self.longitude+'","'+self.latitude+'")'
				print(sql_error)
	...
	try:
		...
		for qimg in qimgs:
			...
			if sql_qimg(self,qimg_url):
				...
				self.c.execute("INSERT INTO IMAGE (CID,RANK,URL) VALUES ('','','"+qimg_url+"')");
				self.conn.commit()
	except:
...
def get_pi(url):
	try:
		pattern = re.compile(r'pi=(\d+)', re.S)
		indexs = re.findall(pattern, url)[0]
		for index in indexs:
			pi = pi + index
	except:
		pi = "1"
