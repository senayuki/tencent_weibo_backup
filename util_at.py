...
def sql_insert(self):
	flag=False#判断数据库中是否已存在
	self.excursor = self.c.execute("SELECT * FROM AT WHERE CID == " + self.cid)
	for row in self.excursor:
		flag=True
	if flag==False:
		try:
			self.c.execute("INSERT INTO AT (CID,TID,TYPE,AUTHOR,CONTENT,TIME,QAUTHOR,QCONTENT,QTIME,MOOD,LOCATION,LONGITUDE,LATITUDE) VALUES ("+self.cid+","+self.tid+","+str(self.type)+",'"+self.author+"','"+self.content+"','"+self.time+"','"+self.qauthor+"','"+self.qcontent+"','"+self.qtime+"','"+self.mood+"','"+self.location+"','"+self.longitude+"','"+self.latitude+"')");#用''引
		except:
			try:
				...
				sql_error = 'INSERT INTO AT (CID,TID,TYPE,AUTHOR,CONTENT,TIME,QAUTHOR,QCONTENT,QTIME,MOOD,LOCATION,LONGITUDE,LATITUDE) VALUES ('+self.cid+','+self.tid+','+str(self.type)+',"'+self.author+'","'+self.content+'","'+self.time+'","'+self.qauthor+'","'+self.qcontent+'","'+self.qtime+'","'+self.mood+'","'+self.location+'","'+self.longitude+'","'+self.latitude+'")'
				print(sql_error)
